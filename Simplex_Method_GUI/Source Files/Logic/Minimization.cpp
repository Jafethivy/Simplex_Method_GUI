...
bool start_minimization(Matrix& m, callback_itr callback) {
    Iteration itr;
    fill_slack_min(m);
    ...
    while (!optimal_solution_min(m) && !unbounded) {
        pivoting_min(m, itr, unbounded);
        if (!unbounded) {
            m.iteration_setter();
            itr.table = m.table_getter();
            callback(itr);
...
void pivoting_min(Matrix& m, Iteration& itr, bool& unbounded) {
    ...
    if (piv_row == -1) {
        unbounded = true;
        return; // Problema no acotado detectado
    }
...
int det_piv_row_min(Matrix& m, int piv_col) {
    ...
    double min_ratio = m.get_value(first_valid_row, rhs_col) / m.get_value(first_valid_row, piv_col);
